...
int main()
{
    ...
    // Perform Scheduling Calculations for FCFS
if (!round_robin_algorithm)
{
    ...
    for (i = 0; i < num_processes; i++)
    {
        ...
        wait_time = completion_time[i] - arrival_time[i];
        turnaround_time = wait_time + burst_time[i];

        average_wait_time += wait_time;
        average_turnaround_time += turnaround_time;

        printf("\nProcess[%d]\t\t%d\t\t\t%d\t\t\t%d\t\t\t%d", i + 1, arrival_time[i], burst_time[i], turnaround_time, wait_time);
    }
 ...
 else
{
    ...
    // Variables to track process completion status
    ...
    // Perform round-robin scheduling calculations
    while (completed_processes < num_processes)
    {
        for (i = 0; i < num_processes; i++)
        {
            if (!completed[i] && arrival_time[i] <= current_time)
            {
